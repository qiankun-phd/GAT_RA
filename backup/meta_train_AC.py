...
for k in range(len(n_veh)):
    env = Environment_marl_general.Environ(n_veh[k], n_neighbor, n_RB, n_BS)
    env.new_random_game()
    envs.append(env)
    ...
    for k in range(len(n_veh)):
        ...
        for i in range(n_veh[k]):
            for j in range(n_neighbor):
                state_all.append(get_state(envs[k], [i, j], n_veh[k], i_episode))
        r_sum = 0
        trans_all_user = []
        ...
        for step in range(T_TIMESTEPS):
            ...
            v_pred_all = []
            reward_all = []
            action_all_training = np.zeros([n_veh[k], n_neighbor, 3], dtype='float32')
            ...
            for i in range(n_veh[k]):
                for j in range(n_neighbor):
                    ...
                    action_all_training[i, j, 0] = channel_action
            ...
            for i in range(n_veh[k]):
                for j in range(n_neighbor):
                    ...
                    state_ = get_state(envs[k], [i, j], n_veh[k], i_episode)
                    state_all[i] = state_
...
model_path = args.save_path + 'AC_' +'%s_' %sigma_add + '%d_' % meta_episode +'%s_' %args.lr_meta_a
save_models(sess, model_path, ppo.saver)

np.savetxt('./Train_data/Meta_Reward_'+'%s_' %sigma_add + '%d_' %meta_episode +'%s_' %args.lr_meta_a, record_reward)
