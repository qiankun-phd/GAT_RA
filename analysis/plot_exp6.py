...
# -*- coding: utf-8 -*-
"""
绘制 exp6 结果：Meta(area=100) -> Train(area=25)，n_RB=16，n_veh=2,4,6,8,10
- TensorBoard 日志: logs/exp6/
- 输出: figures/exp6_convergence_nveh{n}.png/pdf, figures/exp6_summary_nveh.png/pdf
- 合并: figures/exp6_all.pdf
用法: conda run -n RA_demo python analysis/plot_exp6.py
"""
...
_TB_DIR = _PROJECT_ROOT / 'logs' / 'exp6'
...
def _plot_summary(x_list, data, save_path):
    ...
    ax.set_title('Exp6: Meta(area=100) -> Train(area=25), n_RB=16', fontsize=16, fontweight='bold')
    ax.tick_params(axis='both', labelsize=12)
    ax.legend(fontsize=12, loc='best', frameon=True, shadow=False)
    ax.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
...
def _merge_all_pdf():
    pdf_files = []
    for n in N_VEH_LIST:
        p = _FIGURES_DIR / f'exp6_convergence_nveh{n}.pdf'
        if p.exists():
            pdf_files.append(p)
    summary_pdf = _FIGURES_DIR / 'exp6_summary_nveh.pdf'
    if summary_pdf.exists():
        pdf_files.append(summary_pdf)
    if not pdf_files:
        print("  无 PDF 可合并")
        return
    out_path = _FIGURES_DIR / 'exp6_all.pdf'
    try:
...
def main():
    ...
    print("=" * 80)
    print("绘制 exp6: Meta(area=100) -> Train(area=25), n_RB=16, n_veh=2,4,6,8,10")
    ...
    for n in N_VEH_LIST:
        ...
        if len(configs) >= 2:
            _plot_convergence(configs, f'Exp6: Meta(area=100)->Train(area=25), n_RB=16, n_veh={n}',
                              _FIGURES_DIR / f'exp6_convergence_nveh{n}.png')

    if summary_data:
        _plot_summary(N_VEH_LIST, summary_data, _FIGURES_DIR / 'exp6_summary_nveh.png')
