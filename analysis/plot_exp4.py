...
# -*- coding: utf-8 -*-
"""
绘制 exp4 结果：Meta(n_RB=16, area=100) -> Train(area=25)，n_veh=2,4,6,8,10
- TensorBoard 日志: logs/exp4/
- 输出: figures/exp4_convergence_nveh{n}.png/pdf, figures/exp4_summary_nveh.png/pdf
- 合并: figures/exp4_all.pdf
用法: conda run -n RA_demo python analysis/plot_exp4.py
"""
...
_TB_DIR = _PROJECT_ROOT / 'logs' / 'exp4'
...
def _find_logs_for_nveh(n_veh):
    if not _TB_DIR.exists():
        return {}
    result = {}
    pat = re.compile(r'_(\d{8}_\d{6})$')
    tag = f'{EXP_TAG}_nveh{n_veh}'
...
def _plot_summary(x_list, data, save_path):
    ...
    ax.set_xlabel('n_veh', fontsize=14, fontweight='bold')
    ax.set_ylabel('Final Total SEE (last 10% mean)', fontsize=14, fontweight='bold')
    ax.set_title('Exp4: Meta(n_RB=16, area=100) -> Train(area=25)', fontsize=16, fontweight='bold')
    ax.tick_params(axis='both', labelsize=12)
    ax.legend(fontsize=12, loc='best', frameon=True, shadow=False)
    ax.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
...
def _merge_all_pdf():
    pdf_files = []
    for n in N_VEH_LIST:
        p = _FIGURES_DIR / f'exp4_convergence_nveh{n}.pdf'
        if p.exists():
            pdf_files.append(p)
    summary_pdf = _FIGURES_DIR / 'exp4_summary_nveh.pdf'
    if summary_pdf.exists():
        pdf_files.append(summary_pdf)
    if not pdf_files:
        print("  无 PDF 可合并")
        return
    out_path = _FIGURES_DIR / 'exp4_all.pdf'
    try:
...
def main():
    ...
    print("=" * 80)
    print("绘制 exp4: Meta(n_RB=16, area=100) -> Train(area=25), n_veh=2,4,6,8,10")
    print(f"  日志目录: {_TB_DIR}")
    print("=" * 80)

    for n in N_VEH_LIST:
        paths = _find_logs_for_nveh(n)
        if not paths:
            print(f"  跳过 n_veh={n}: 未找到日志")
            continue
        configs = {}
        for cfg, p in paths.items():
            ...
            summary_data[cfg][n] = final
        if len(configs) >= 2:
            _plot_convergence(configs, f'Exp4: Meta(n_RB=16, area=100)->Train(area=25), n_veh={n}',
                              _FIGURES_DIR / f'exp4_convergence_nveh{n}.png')

    if summary_data:
        _plot_summary(N_VEH_LIST, summary_data, _FIGURES_DIR / 'exp4_summary_nveh.png')
